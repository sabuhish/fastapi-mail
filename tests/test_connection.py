...
from fastapi_mail.connection import ConnectionEnum, BaseConnection, TestConnection
...
async def test_connection(mail_config):
    ...
    connection = fm._guess_connection(1, 0)

    assert issubclass(connection, BaseConnection) is True
    assert connection == TestConnection
...
async def test_attachement_message(mail_config):
    ...
    async with fm.async_record_messages() as outbox:
        await fm.send_message(message=msg)
        mail = outbox[0]
...
async def test_send_msg(mail_config):
    msg = MessageSchema(
        subject="testing", recipients=["to@example.com"], body="html test"
    )
